...
Position.prototype.getMarkToBaseAttachment = function(lookupTables, baseGlyphIndex, markGlyphIndex) {
  for (let i = 0; i < lookupTables.length; i++) {
    ...
    for (let j = 0; j < subtables.length; j++) {
      ...
      if (markCovIndex < 0 || baseCovIndex < 0)
        continue;
...
Font.prototype.getFeaturesConfig = function(options) {
  return options ? this.updateFeatures(options.features) : this.defaultRenderOptions.features;
};
...
Font.prototype.getGlyphsPositions = function(glyphs, options) {
  ...
  const features = this.getFeaturesConfig(options).filter((f) => f.script === "DFLT").reduce((tags, feature) => tags.concat(feature.tags), []);
  if (options && options.kerning)
    features.push("kern");
...
subtableParsers2[4] = function parseLookup42() {
  const start = this.offset + this.relativeOffset;
  const posFormat = this.parseUShort();
  check_default.assert(posFormat === 1, "0x" + start.toString(16) + ": GPOS lookup type 4 format must be 1.");
...
subtableParsers2[9] = function parseLookup9() {
  const start = this.offset + this.relativeOffset;
  const posFormat = this.parseUShort();
  check_default.assert(posFormat === 1, "0x" + start.toString(16) + ": GPOS lookup type 9 format must be 1.");
  const extensionLookupType = this.parseUShort();
  return {
    posFormat,
    extensionLookupType,
    extension: this.parsePointer32(subtableParsers2[extensionLookupType])
  };
};
