...
Parser.prototype.parseBaseArray = function (marksClassCount) {
  ...
  return this.parseList(
    count,
    Parser.list(marksClassCount, Parser.pointer(Parser.anchor))
  );
};
  ...
  switch (format) {
    ...
    case 3:
      return {
         ...
         */
        xDevice: this.parseUShort(),
        // offset
        yDevice: this.parseUShort(),
        // offset
      };
  }
  throw new Error(
    "0x" + startOffset.toString(16) + ": Anchor format must be 1, 2 or 3."
  );
};
...
function parseCFFCharstring(font, glyph, code, version) {
  ...
  if (font.isCIDFont || version > 1) {
    ...
    if (version > 1) {
      vstore =
        cffTable.topDict._vstore.itemVariationStore.itemVariationSubtables;
...
function fontToSfntTable(font) {
  ...
  const fvarTable = font.tables.fvar
    ? fvar_default.make(font.tables.fvar, font.names)
    : void 0;
  const gaspTable = font.tables.gasp
    ? gasp_default.make(font.tables.gasp)
    : void 0;
  const languageTags = [];
  ...
  for (let tableName in optionalTables) {
    ...
    if (table) {
      tables.push(
        optionalTables[tableName].make.call(
          font,
          table,
...
Layout.prototype = {
  ...
  getFeaturesLookups: function (requestedFeatures, script, language) {
    ...
    requestedFeatures = this.supportedFeatures.filter((f) =>
      requestedFeatures.includes(f.featureName)
    );
    ...
    for (let idx = 0; idx < requestedFeatures.length; idx++) {
      ...
      if (featureTable && supportedLookups.length) {
        ...
        for (let i = 0; i < lookupListIndexes.length; i++) {
          const idx2 = `idx${lookupListIndexes[i]}`;
          if (Object.prototype.hasOwnProperty.call(lookupUnionList, idx2))
            continue;
          lookupTable = allLookups[lookupListIndexes[i]];
          if (!lookupTable) continue;
          let validLookupType =
            supportedLookups.indexOf(lookupTable.lookupType) !== -1;
          if (lookupTable.subtables.length === 1) {
            const { extensionLookupType, extension } = lookupTable.subtables[0];
            if (
              extensionLookupType &&
              extension &&
              supportedLookups.indexOf(extensionLookupType) !== -1
            ) {
          ...
          if (validLookupType) {
            ...
            lookupUnionList[idx2] = lookupTable;
          }
        }
      }
    }
...
function Position(font) {
  layout_default.call(this, font, "gpos", [
    { featureName: "kern", supportedLookups: [2, 9] },
...
Position.prototype.getKerningValue = function (
  ...
  for (let i = 0; i < kerningLookups.length; i++) {
    ...
    for (let j = 0; j < subtables.length; j++) {
      ...
      const covIndex = this.getCoverageIndex(
        subtable.coverage ?? subtable.extension.coverage,
        leftIndex
      );
...
Position.prototype.getMarkToBaseAttachment = function (
  lookupTables,
  baseGlyphIndex,
  markGlyphIndex
) {
  for (let i = 0; i < lookupTables.length; i++) {
    ...
    for (let j = 0; j < subtables.length; j++) {
      const subtable = subtables[j];
      const markCovIndex = this.getCoverageIndex(
        subtable.markCoverage,
        markGlyphIndex
      );
      const baseCovIndex = this.getCoverageIndex(
...
function IF(state) {
  ...
  if (!test) {
    skip(state, true);
    if (exports.DEBUG) console.log(state.step, ins === 27 ? "ELSE[]" : "EIF[]");
  }
}
function EIF(state) {
...
FeatureQuery.prototype.getSubstitutionFeaturesLookups = function (
  features,
  script,
  language
) {
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    for (let j = 0; j < substitutions.length; j++) {
      ...
      if (action instanceof SubstitutionAction) {
        applySubstitution_default.call(this, action, tokens, j);
  ...
  for (let index = 0; index < contextParams.context.length; index++) {
    ...
    if (substitutions.length) {
      for (let i = 0; i < substitutions.length; i++) {
        ...
        applySubstitution_default.call(this, action, tokens, index);
      }
...
function applySubstitutions(script, features, contextName) {
  const supportedFeatures = features.filter((feature) =>
    this.hasFeatureEnabled(script, feature)
  );
  const featuresLookups = this.query.getSubstitutionFeaturesLookups(
    supportedFeatures,
    script
  );
  for (let idx = 0; idx < featuresLookups.length; idx++) {
    ...
    const ranges = this.tokenizer.getContextRanges(contextName);
    for (let k = 0; k < ranges.length; k++) {
      ...
      for (let index = 0; index < contextParams.context.length; index++) {
        ...
        for (let s = 0; s < subtables.length; s++) {
          ...
          const substType = this.query.getSubstitutionType(
            lookupTable,
            subtable
          );
          ...
          switch (substType) {
            case "11":
              substitution = lookup(contextParams.current);
              if (substitution) {
                applySubstitution_default.call(
                  this,
                  new SubstitutionAction({
                    id: 11,
                    tag: lookupTable.feature,
                    substitution,
                  }),
                  tokens,
                  contextParams.index
...
// src/features/positioning/kern.js
function kern(lookupTable, glyphs) {
  ...
  for (let i = 0; i < glyphs.length; i += 1) {
    ...
    if (i > 0) {
      coords[i] = {
        xAdvance: this.position.getKerningValue(
          [lookupTable],
          glyphs[i - 1].index,
          glyph.index
        ),
...
// src/features/positioning/mark.js
function mark(lookupTable, glyphs) {
  ...
  for (let i = 0; i < glyphs.length; i += 1) {
    ...
    coords[i] = { xAdvance: 0, yAdvance: 0 };
    if (i > 0) {
      const coordinatedPair = this.position.getMarkToBaseAttachment(
        [lookupTable],
        glyphs[i - 1].index,
        glyph.index
      );
      if (coordinatedPair) {
        ...
        coords[i] = {
          xAdvance:
            baseMarkPoint.xCoordinate -
            attachmentMarkPoint.xCoordinate -
            glyphs[i - 1].advanceWidth,
...
Font.prototype.forEachGlyph = function (
  ...
  for (let i = 0; i < glyphs.length; i += 1) {
    ...
    callback.call(
      this,
      glyph,
      x + xAdvance * fontScale,
      y + yAdvance * fontScale,
      fontSize,
      options
    );
...
Font.prototype.getGlyphsPositions = function (glyphs, options) {
  ...
  const featuresLookups = this.position.getPositionFeatures(
    features,
    script,
    options.language
  );
  for (let i = 0; i < featuresLookups.length; i++) {
    ...
    for (let index = 0; index < pos.length; index++) {
      ...
      if (lookupTable.feature === "kern") {
        kerningValue += glyphPosition.xAdvance;
  ...
  if (options.kerning && kernLookupTableProcessed === false) {
    let kerningValue = 0;
    for (let i = 1; i < glyphs.length; i += 1) {
      kerningValue += this.getKerningValue(glyphs[i - 1], glyphs[i]);
...
subtableParsers2[4] = function parseLookup42() {
  ...
  check_default.assert(
    posFormat === 1,
    "0x" + start.toString(16) + ": GPOS lookup type 4 format must be 1."
  );
...
subtableParsers2[9] = function parseLookup9() {
  ...
  check_default.assert(
    posFormat === 1,
    "0x" + start.toString(16) + ": GPOS lookup type 9 format must be 1."
  );
  const extensionLookupType = this.parseUShort();
  return {
    posFormat,
    extensionLookupType,
    ...this.parsePointer32(subtableParsers2[extensionLookupType]),
  };
};
